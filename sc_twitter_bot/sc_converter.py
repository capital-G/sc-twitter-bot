...
class SuperColliderConverter:
    def __init__(
        ...
        try:
            ...
            raise ConverterException(f'Please check sclang path or install supercollider! {e}')

        try:
            subprocess.check_output([self.scsynth_path, '-v'])
        except (subprocess.CalledProcessError, FileNotFoundError) as e:
            raise ConverterException(f'Please check scsynth path or install supercollider! {e}')
    ...
    def _osc_to_wav(
        ...
        log.info(f'Successfully converted {osc_file} to {wav_file} in {(datetime.now() - start_time).seconds} seconds')
