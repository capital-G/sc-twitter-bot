...
class TwitterBotTestCase(TestCase):
    ...
    def test_filter_synth_def(self):
        ...
        self.assertEqual(
            self.twitter_bot._filter_out_synth_def(MagicMock(full_text='@sc2sbot {SinOsc.ar}', mentions={})),
            '{SinOsc.ar}'
        )

        # no name (although this should not happen)
        self.assertEqual(
            self.twitter_bot._filter_out_synth_def(MagicMock(full_text='{SinOsc.ar}', mentions={})),
            '{SinOsc.ar}',
        )

        # name at the end
        self.assertEqual(
            self.twitter_bot._filter_out_synth_def(MagicMock(full_text='SinOsc.ar @sc2sbot', mentions={})),
            'SinOsc.ar'
        )
